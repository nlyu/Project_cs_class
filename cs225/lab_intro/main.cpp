    ...
    for (size_t yi = 0; yi < original.height(); yi++) {
        for (size_t xi = 0; xi < original.width(); xi++) {
            /// Your code here!
                ...
                if( original(yi, xi)->red + amount > 255){
                	original(yi, xi)->red = 255;
                }
		else{
			original(yi, xi)->red += amount;
		}
	
                if( original(yi, xi)->blue + amount > 255){
                	original(yi, xi)->blue = 255;
                }
		else{
                        original(yi, xi)->blue += amount;
                }

               	if( original(yi, xi)->green + amount > 255){
                	original(yi, xi)->green = 255;
                }
		else{
                        original(yi, xi)->green += amount;
                }
...
{
    /// Your code here!
    for(size_t row = 0; row < firstImage.height(); ++row){
    	for(size_t col = 0; col < firstImage.height(); ++col){
    	
    		firstImage(row, col)->red = (firstImage(row, col)->red + secondImage(row, col)->red)/2;
    		firstImage(row, col)->green = (firstImage(row, col)->green + secondImage(row, col)->green)/2;
    		firstImage(row, col)->blue = (firstImage(row, col)->blue + secondImage(row, col)->blue)/2;

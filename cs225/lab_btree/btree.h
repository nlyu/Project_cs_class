...
size_t insertion_idx_helper(const std::vector<T>& v, const C val, int mid, int start, int end)
{
	if(start == end){
	...
	if(val < v[mid]){
		return insertion_idx_helper(v, val, (mid + start)/2, start, mid - 1);
	}
	return insertion_idx_helper(v, val, ((end + mid)/2) == end ? ((end + mid)/2) : ((end + mid)/2+1), mid, end);
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if(size == 0)	return 0;
    auto result = insertion_idx_helper(elements, val, size/2, 0, size-1);
    return result;
}

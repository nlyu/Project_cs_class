...
class BinaryTree
{
  ...
  public:
    ...
    void mymirror(Node* &myroot);
    
    int myisordered(const Node* myroot, bool * myorder) const;
    
    void printpath(const Node * myroot, vector<T> & path) const;
    
    int sum(const Node * myroot, int level) const;

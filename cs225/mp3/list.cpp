...
typename List<T>::ListNode* List<T>::merge(ListNode* first, ListNode* second)
{
	...
	{
		if(c1->data < c2->data)//compare c1 and c2, if c1 is smaller, move to the next one;
		{
			if(c1->next == NULL){
				c1Prev = c1;
				}
	...
	//append c2 to the tail of the c1
	if(c1 == NULL)
	{
		c1Prev->next = c2;
		c2->prev = c1Prev;
...
typename List<T>::ListNode* List<T>::mergesort(ListNode* start, int chainLength)
{
	...
	int rest_length = chainLength-chainLength/2;
	other = mergesort(other, rest_length);	

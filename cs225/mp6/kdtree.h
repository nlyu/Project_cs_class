...
class KDTree
{
  ...
  private:
     ...
     */
    Point<Dim> helper_find(int left, int right, int dim, int & min_dim, const Point<Dim>& q) const;
Point<Dim> helper_find(int left, int right, int dim, const Point<Dim>& q) const;
void buildtree(vector<Point<Dim>>& newPoints,int left,int right,int curdim);
int partition(vector<Point<Dim>>&Points , int left, int right, int pivotIndex,int dim);
void swap(vector<Point<Dim>>&Points,int a,int b);
Point<Dim> select(vector<Point<Dim>>& list, int left,int right,int n,int dim);
};

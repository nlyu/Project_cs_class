                                 ...
                                 int tolerance, int frameFreq)
{
     ...
     */
     solidColorPicker pick(fillColor);
                                ...
                                int gridSpacing, int tolerance, int frameFreq)
{
     ...
     * correct call to fill with the correct colorPicker parameter.
     */
     gridColorPicker pick(gridColor, gridSpacing);
    return fill(img, x, y, pick, tolerance, frameFreq);
}
                                    ...
                                    int radius, int tolerance, int frameFreq)
{
     ...
     * correct call to fill with the correct colorPicker parameter.
     */
        gradientColorPicker pick(fadeColor1, fadeColor2, radius, x, y);
	return fill(img, x, y, pick, tolerance, frameFreq);
}
                       ...
                       int tolerance, int frameFreq)
{
	...
	int **done = new int *[width];
	for(int i = 0; i < width; ++i){
     		done[i] = new int[height];
     		for(int j = 0; j < height; ++j){
     			done[i][j] = 0;
    	 	}
   	}
	OrderingStructure<int> xcoord;//x coordinates
	OrderingStructure<int> ycoord;// y coordinates
	...
	while(xcoord.isEmpty() == false && ycoord.isEmpty() == false ){
		//pop
		currx = xcoord.remove();
		curry = ycoord.remove();
		...
		//update
		if((pow(img(currx, curry)->red - origin.red, 2) + pow(img(currx, curry)->blue-origin.blue, 2)+pow(img(currx, curry)->green - origin.green,2))>tolerance)
			continue;
		if(img(currx, curry) != NULL && !done[currx][curry]){

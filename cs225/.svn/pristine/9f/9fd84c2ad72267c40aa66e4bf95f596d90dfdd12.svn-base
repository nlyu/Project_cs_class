...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        if(whenVisitedTable.keyExists(ll.customer+ll.url)){
            if(ll.date>whenVisitedTable[ll.customer+ll.url])
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
    ...
    //for(auto i = whenVisitedTable.begin(); i != whenVisitedTable.end(); i++){

    //}
    bool visited = whenVisitedTable.keyExists(customer+url);
    return  visited;// replaceme
}
                                  ...
                                  const string& url) const
{
    ...
    //(void) customer; // prevent warnings... When you implement this function, remove this line.
    //(void) url;      // prevent warnings... When you implement this function, remove this line.
    //string unique = customer+url;
    time_t mydate = whenVisitedTable.find(customer+url);
    return mydate; // replaceme
}

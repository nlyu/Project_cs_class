...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string w1 = word1;
    string w2 = word2;
    //set words to uppercase
    std::transform(w1.begin(), w1.end(), w1.begin(), toupper);
    std::transform(w2.begin(), w2.end(), w2.begin(), toupper);
    //get two word and cmp them 
    auto fine_1 = dict.find(w1);
    auto fine_2 = dict.find(w2);
    if(fine_1 == dict.end() || fine_2 == dict.end())
    {
    	return false;
    }
    if(fine_1->second != fine_2->second){
	return false;
    }		
    //pass the test
    return true;

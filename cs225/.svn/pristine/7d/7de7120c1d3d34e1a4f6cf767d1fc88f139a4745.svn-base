...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(key == subroot->elements[first_larger_idx].key)   return subroot->elements[first_larger_idx].value;
    if(subroot->children.size() == 0)    return V();
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /* TODO Your code goes here! */
    //step 1 insert into parent
    parent->elements.insert(elem_itr, child->elements[mid_elem_idx]);
    //cout << *parent << endl;
    //step 2 split child and make new right
    ...
    //cout << *new_right << endl;
    //step 3 delete element in the left
    //cout << *new_left <<endl;
    //insert left child into parent
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    auto parent_st = subroot->elements.begin();
    auto children_st = subroot->children.begin();
    /* TODO Your code goes here! */
    if(subroot->is_leaf){   
        subroot->elements.insert(parent_st + first_larger_idx, pair);
        return;
    }
    //goto the bottom root    
    insert(subroot->children[first_larger_idx], pair);
    auto sizechild = subroot->children[first_larger_idx]->elements.size();
    //check size
    if(sizechild >= order){   

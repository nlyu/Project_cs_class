...
void SCHashTable<K, V>::resizeTable()
{
     ...
     * @hint Use findPrime()!
     */
    size_t originalsize = 2*size;
    size_t newsize = findPrime(originalsize);
    list<std::pair<K, V>>* newtable;
    newtable = new list<pair<K, V>>[newsize];
    for(size_t i = 0; i<size; i++){
        for (auto iter = table[i].begin(); iter!= table[i].end(); iter++) {
            size_t idx = hash((*iter).first, newsize);
            //while (newtable[idx] != NULL)
                //idx = (idx + 1) % newSize;
            newtable[idx].push_front(*iter);
